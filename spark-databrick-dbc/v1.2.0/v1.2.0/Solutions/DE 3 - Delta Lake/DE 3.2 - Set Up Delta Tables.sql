...
-- MAGIC
-- MAGIC  
-- MAGIC ## Declare Schema with Generated Columns and Provide Table Properties
-- MAGIC
...
-- MAGIC 1. Providing table properties, which include `created.by.user`, `created.date` and `delta.appendOnly` 
-- MAGIC
-- MAGIC By setting the appendOnly property to true, we will not be able to update or delete records from this table.
...
-- DBTITLE 0,--i18n-656802b8-b312-4425-b4c6-e4f0c8390ff5
-- MAGIC %md
-- MAGIC ## Attempt to Delete a Record
-- MAGIC
-- MAGIC When we created the table, we added some `TBLPROPERTIES`, one of which was setting `delta.appendOnly` to `true`. If we attempt to delete records, we get an error. Uncomment the following cell and run it to show this error.

-- COMMAND ----------

-- DELETE FROM purchase_dates WHERE 'id' = 257436;
